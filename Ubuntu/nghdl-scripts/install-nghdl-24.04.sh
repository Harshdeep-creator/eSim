...
{   
    ...
    # Skip rebuild when a working GHDL installation already exists.
    # Avoids several minutes of unnecessary recompilation during
    # repeated installer runs on Ubuntu 25.04.
    if command -v ghdl >/dev/null 2>&1 && ghdl --version >/dev/null 2>&1; then
        echo "GHDL is already installed on this system:"
        ghdl --version | head -n 1
        echo "Skipping GHDL compilation."
        return 0
    fi
    ...
    echo "Patching configure for LLVM 20.1.x..."
    sed -i '/check_version 18.1 \$llvm_version/a\
    check_version 20.1 $llvm_version ||' configure
    grep -A2 "18.1" configure
